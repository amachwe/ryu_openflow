...
class FlowInstaller(app_manager.RyuApp):
    ...
    def event_dp(self, ev):
        ...
        if ev.enter and not switch_db.get(ev.dp.address):
            ...
            print(sw_internet_port, ev.ports)
            if not sw_internet_port:
                return

            for port in ev.ports:
                if port != sw_internet_port:
                    print("Added flow to: {}:{}\t\tinternet port:{}".format(ev.dp.id, port, sw_internet_port))
                    FlowFactory.add_bridge_flow(ev.dp, port.port_no, sw_internet_port.port_no)

            ports = list(filter(lambda x: x!= sw_internet_port, ev.ports))

            FlowFactory.add_rev_bridge_flow(ev.dp, sw_internet_port, ports)
        else:
...
class FlowFactory(object):
    ...
    @staticmethod
    def add_bridge_flow(datapath, port_in, port_out, pin=True):
